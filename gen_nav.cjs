...
import { Home, Wallet, PiggyBank, Calendar, Menu, FileText, Users, Scale, HeartPulse, Sparkles, Settings, Sprout, Car, X } from 'lucide-react';
...
export function BottomNav() {
  ...
  const moreItems = [
    ...
    { href: '/vehicles', label: '🚗 Vehicles & Garage', icon: Car, desc: 'Car/Bike details, Service & PUC' },
    ...
    <>
      ...
      {isMoreOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-navy-dark/60 backdrop-blur-sm">
          <div className="w-full max-w-[430px] bg-paper rounded-t-3xl sm:rounded-3xl shadow-2xl p-5 border border-paper-dim space-y-4 max-h-[82vh] overflow-y-auto">
...
fs.writeFileSync('components/ui/BottomNav.tsx', navCode.trim() + '\n', 'utf8');
console.log('Saved components/ui/BottomNav.tsx');
