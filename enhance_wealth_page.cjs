...
const wealthPageCode = `'use client';
...
import { TrendingUp, RefreshCw, Plus, Sparkles, Coins, Landmark, Building, ArrowUpRight, ArrowDownRight, Layers } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from 'recharts';
import confetti from 'canvas-confetti';

export default function WealthPage() {
  const { assets, totalWealth, liquidWealth, fixedWealth, goals, addAsset } = useFamilyStore();
  ...
  const [marketRates, setMarketRates] = useState<{
    ...
    mutual_funds: {
      'PPFAS': { nav: 82.45, schemeName: 'Parag Parikh Flexi Cap Fund' },
      'QUANT': { nav: 265.12, schemeName: 'Quant Small Cap Fund' }
    },
  ...
  const [assetType, setAssetType] = useState<'bank_deposit' | 'gold' | 'shares' | 'mutual_fund' | 'land' | 'property'>('shares');
  ...
  const pieData = [
    { name: 'Liquid (Bank/Shares/MF)', value: liquidWealth, color: '#4C7A5E' },
    { name: 'Fixed (Gold/Land/House)', value: fixedWealth, color: '#B98B2A' },
  ];
  ...
  const handleAddAssetSubmit = (e: React.FormEvent) => {
    ...
    if (assetType === 'shares' && qty && buyPrice) {
      computedVal = qty * (marketRates.stocks[assetSymbol.toUpperCase()]?.price || buyPrice);
    } else if (assetType === 'gold' && qty) {
      // qty in grams
      computedVal = qty * marketRates.gold_24k_per_g;
    }

    addAsset({
      category: assetCat,
      type: assetType as any,
      label: assetLabel || (assetType === 'shares' ? assetSymbol + ' Shares' : assetType === 'gold' ? qty + 'g Gold' : 'Asset'),
    ...
    <div className="space-y-4">
        ...
        <div className="p-3.5 bg-paper rounded-2xl border border-paper-dim shadow-sm space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5 pt-1 text-[11px]">
            ...
            <div className="p-2 bg-paper-dim/60 rounded-xl border border-paper-dim">
              ...
              <Mono className="font-bold text-ink text-xs block mt-0.5">
                NAV ₹{marketRates.mutual_funds['PPFAS']?.nav}
              </Mono>
      ...
      <div className="px-4 space-y-2.5">
        {filteredAssets.map((asset) => {
          const isShare = asset.type === 'shares' || asset.type === 'mutual_fund';
      ...
      {isAddAssetOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-dark/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-paper p-5 rounded-2xl border border-paper-dim shadow-xl space-y-3 max-h-[85vh] overflow-y-auto">
            <h3 className="text-sm font-bold font-serif text-ink">Naya Wealth / Asset Jodein</h3>
            <form onSubmit={handleAddAssetSubmit} className="space-y-3">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={'py-2 text-xs font-bold rounded-xl border ' + (assetCat === 'liquid' ? 'bg-green text-white border-green' : 'bg-paper text-ink-muted border-paper-dim')}
                  >
                    💧 Liquid (Shares/Bank/MF)
                  </button>
                  <button
                    type="button"
                    onClick={() => { setAssetCat('fixed'); setAssetType('gold'); }}
                    className={'py-2 text-xs font-bold rounded-xl border ' + (assetCat === 'fixed' ? 'bg-gold text-white border-gold' : 'bg-paper text-ink-muted border-paper-dim')}
                  >
                    🏛️ Fixed (Gold/Land)
                  </button>
                </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Reliance Shares, Parag Parikh MF, Gold Locker"
                  value={assetLabel}
              ...
              {assetType === 'shares' && (
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    <label className="text-[9px] font-bold uppercase text-ink-muted block mb-1">Symbol</label>
                    <input
                      type="text"
                      placeholder="RELIANCE"
                      value={assetSymbol}
                      onChange={(e) => setAssetSymbol(e.target.value)}
                      className="w-full px-2 py-1.5 text-xs bg-paper-dim border border-paper-dim rounded-lg font-mono uppercase"
                    />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold uppercase text-ink-muted block mb-1">Quantity</label>
                    <input
                      type="number"
                      placeholder="50"
                      value={assetQty}
                  ...
                  <div>
                    <label className="text-[9px] font-bold uppercase text-ink-muted block mb-1">Buy Price</label>
                    <input
                      type="number"
                      placeholder="2400"
                      value={assetBuyPrice}
              ...
              {assetType === 'gold' && (
                <div>
                  <label className="text-[10px] font-bold uppercase text-ink-muted block mb-1">Weight (Grams me)</label>
                  <input
                    type="number"
                    ...
                    onChange={(e) => {
                      ...
                      setAssetVal((g * marketRates.gold_24k_per_g).toString());
                    }}
                    className="w-full px-3 py-2 text-xs bg-paper-dim border border-paper-dim rounded-xl font-mono"
                  />
                  <span className="text-[10px] text-gold mt-1 block">Live Rate: ₹{marketRates.gold_24k_per_g}/gram</span>
                </div>
              )}

              <div>
                <label className="text-[10px] font-bold uppercase text-ink-muted block mb-1">Estimated / Total Value (₹)</label>
                <input
                  type="number"
...
fs.writeFileSync('app/(dashboard)/wealth/page.tsx', wealthPageCode.trim() + '\n', 'utf8');
console.log('Saved enhanced app/(dashboard)/wealth/page.tsx');
