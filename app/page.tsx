...
import { 
  ...
  ChevronRight, BarChart3, Star, Download, Coins, LogOut
} from 'lucide-react';
import { useFamilyStore } from '@/lib/store/familyStore';

export default function LandingPage() {
  const { isLoggedIn, authUser, logout, family } = useFamilyStore();
  const userName = authUser?.user_metadata?.full_name || authUser?.email?.split('@')[0] || 'Parivar Head';
    ...
    <div className="min-h-screen bg-[#0B0F19] text-slate-100 font-sans selection:bg-gold/30 selection:text-white">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0B0F19]/80 border-b border-slate-800/80 px-4 md:px-8 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5 sm:gap-4">
          {isLoggedIn ? (
            <>
              <div className="hidden sm:flex items-center gap-2 text-xs font-semibold text-amber-300 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl">
              ...
              <button
                onClick={logout}
                className="text-xs font-semibold text-slate-400 hover:text-white px-2.5 py-2 rounded-xl hover:bg-slate-800 transition-colors"
                title="Sign Out"
              >
                <LogOut size={15} />
              </button>
      ...
      <section className="relative pt-12 pb-20 px-4 md:px-8 max-w-6xl mx-auto text-center space-y-8 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 pt-2">
          {isLoggedIn ? (
            <Link
              href="/home"
              className="w-full sm:w-auto px-8 py-4 rounded-2xl bg-gradient-to-r from-amber-400 via-amber-300 to-yellow-400 text-navy font-black text-base shadow-xl shadow-amber-400/20 hover:shadow-amber-400/30 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
              <span>🏠 Apne Parivar Ka Vault Kholein (Dashboard)</span>
              <ArrowRight size={18} />
            </Link>
          ) : (
            <>
